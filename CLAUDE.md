...
- Medido en la PC de desarrollo con worker real: 5.000 filas en 192 s, lotes de ~20 s y ~55 MB; matar el worker a mitad del lote 3 → reintento (intento 2) sin duplicados. Lotes `tries = 3`, `timeout = 300`: **`DB_QUEUE_RETRY_AFTER` tiene que ser mayor (600 en producción)**. Con una sola cola, mientras importa se demoran los demás jobs (autorización de facturas pendientes) un lote como mucho.
