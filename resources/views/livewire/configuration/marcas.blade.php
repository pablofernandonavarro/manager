<div class="space-y-6">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-700">Administrá las marcas disponibles para los productos</p>
        </div>
    </div>

    <!-- Formulario nueva marca -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h3 class="text-base font-semibold text-gray-900 mb-4">Nueva marca</h3>
        <div class="flex gap-2">
            <input type="text"
                   wire:model="nombre"
                   wire:keydown.enter.prevent="save"
                   placeholder="Ej: Adidas, Nike, Zara..."
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($marcas as $marca)
                    <tr wire:key="marca-{{ $marca->id }}" class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            @if($editingId === $marca->id)
                                <input type="text"
                                ...
                                @enderror
                            @else
                                <span class="text-sm font-medium text-gray-900 {{ !$marca->activo ? 'line-through text-gray-400' : '' }}">
                                    {{ $marca->nombre }}
                                </span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-center">
                            <button type="button" wire:click="toggleActive({{ $marca->id }})"
                                    class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none {{ $marca->activo ? 'bg-blue-600' : 'bg-gray-200' }}">
                                <span class="inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform {{ $marca->activo ? 'translate-x-4.5' : 'translate-x-0.5' }}"></span>
                            </button>
                        </td>
                        <td class="px-6 py-4 text-right">
                            ...
                            @else
                                <div class="flex items-center justify-end gap-1">
                                    <button type="button" wire:click="startEdit({{ $marca->id }})"
                                            class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </button>
                                    <button type="button" wire:click="delete({{ $marca->id }})"
                                            wire:confirm="¿Eliminar la marca '{{ $marca->nombre }}'?"
