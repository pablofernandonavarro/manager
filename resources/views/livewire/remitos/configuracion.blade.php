    ...
    <form wire:submit="guardar" class="space-y-8">
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                ...
                <button type="button" wire:click="$toggle('rutaDirecta')" class="relative inline-flex h-8 w-14 items-center rounded-full transition-colors {{ $rutaDirecta ? 'bg-blue-600' : 'bg-gray-300' }}">
                    <span class="inline-block h-6 w-6 transform rounded-full bg-white transition-transform {{ $rutaDirecta ? 'translate-x-7' : 'translate-x-1' }}"></span>
                </button>
            </div>
