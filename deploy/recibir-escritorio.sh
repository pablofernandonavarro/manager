...
#
#   ssh deploy@servidor "<version> <sha256>" < pos-escritorio.zip
#
# Antes de publicar verifica que el archivo llegó entero (sha256), que es un zip sin datos de
# ninguna caja y que la versión es mayor que la publicada (las cajas corren las migraciones
# solo cuando la versión sube). El reemplazo es atómico: nunca queda un zip a medio subir.
set -euo pipefail
...
read -r VERSION SHA EXTRA <<<"${SSH_ORIGINAL_COMMAND:-}"
[[ -z "${EXTRA:-}" ]] || falla 'Uso: "<version> <sha256>" con el zip por stdin.'
[[ "${VERSION:-}" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || falla "Versión inválida: ${VERSION:-vacía}"
[[ "${SHA:-}" =~ ^[0-9a-f]{64}$ ]] || falla 'sha256 inválido.'

install -d -m 2775 "$DIR"
ACTUAL=$(sed -n 's/.*"version": *"\([^"]*\)".*/\1/p' "$DIR/pos-escritorio.json" 2>/dev/null || true)
...
chmod 664 "$TMP"
mv -f "$TMP" "$DIR/pos-escritorio.zip"
trap - EXIT
rm -f "$DIR/pos-escritorio.exe"

printf '{\n    "version": "%s",\n    "formato": "zip",\n    "tamano": %s,\n    "sha256": "%s",\n    "generado_at": "%s"\n}\n' \
    "$VERSION" "$TAMANO" "$SHA" "$(date -u +%Y-%m-%dT%H:%M:%S+00:00)" > "$DIR/pos-escritorio.json.tmp"
chmod 664 "$DIR/pos-escritorio.json.tmp"
mv -f "$DIR/pos-escritorio.json.tmp" "$DIR/pos-escritorio.json"

echo "Publicada la app de escritorio $VERSION ($((TAMANO / 1024 / 1024)) MB)."
